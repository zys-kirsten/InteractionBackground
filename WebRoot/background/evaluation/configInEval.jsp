...
<body>
	 <br>
	 <form action="${ctxPath }/CommitInEvalWeight.do"  method="post">
		 <div class="menudiv">
	       <!-- 组内评价设置 start -->
		  <div id="con_one_5">
		     <table width="82%" border="0" cellpadding="0" cellspacing="0" class="CContent">
		         <tr>
				  <td style="text-align:center;">组内评价设置</td>
				 </tr>
				 <tr>
				  <td>
				    <table class="table table-bordered table-hover m10" style="margin-left:10px;margin-top:3px;">
				      ...
				      <th style="text-align:center;">
				        <c:if test="${not empty sessionScope.elementsinews.get(0).evaluationelement.eeid}">
				          <input type="hidden" name="fatherId" value="${sessionScope.elementsinews.get(0).evaluationelement.eeid}">
				        </c:if>
				      </th>
				      <c:forEach items="${sessionScope.elementsinews}" var="es">
					  ...
					  <tr>
				        ...
				        <c:forEach items="${sessionScope.elementsinews}" var="e">
				          <c:choose>
				             ...
				             <c:otherwise>
				                <td style="text-align:center;">
							        <select name="inEval" style="width:100%;">
							          <option value ="1.0" selected="selected">1</option>
							          <option value ="2.0" >2</option>
									  <option value ="3.0" >3</option>
									  <option value ="4.0" >4</option>
				      ...
				      <tr>
				        <c:choose>
				          <c:when test="${not empty sessionScope.inmsg}">
				            <td class="tableleft" style="text-align:center;" width="150px;">
				               <c:out value="${sessionScope.inmsg}"></c:out>
				            </td>
				              <c:forEach items="${sessionScope.elementsinews}" var="es">
