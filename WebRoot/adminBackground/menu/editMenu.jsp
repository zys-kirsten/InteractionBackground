...
<script type="text/javascript">
 $(document).ready(function(){
	 var kk = document.getElementById("fatherId").options;  
	 for (var j=0; j<kk.length; j++) {  
		 if (kk[j].value=='${menuVo.fatherId}') {  
			 kk[j].selected=true;  
			 break;  
		 }  
	 }
				 ...
				 <tr>
				  <td>
				    <table class="table table-bordered table-hover m10" style="margin-left:10px;margin-top:3px;">
				      <c:set var="t" scope="session" value="${menuVo}"/>
                        <c:if test="${not empty t}">
                        <input type="hidden" name="mid" value="${menuVo.mid }">
					      <tr>
					        <td style="text-align:center;">
					                   菜单名称
					        </td>
					        <td style="text-align:center;">
	                          <input type="text" name="mname" value="${menuVo.mname }">
	                        </td>
	                        <td style="text-align:center;">
					                  父级菜单
					        </td>
